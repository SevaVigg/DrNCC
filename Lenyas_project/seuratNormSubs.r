...
result_table <- data.frame(matrix(ncol=length(colnames(logExps)), nrow = 0))
colnames(result_table) <- colnames(logExps)
bootstrap <- sort(sample(colnames(logExps), size = length(logExps)*0.2))
print("!!!!!!!!!!!!!")
print(bootstrap)
print("!!!!!!!!!!!!!")
logExps<-logExps[,!(colnames(logExps) %in% bootstrap)]
Cells <- Cells[,!(colnames(logExps) %in% bootstrap)]
allGenes <- rownames(Cells[,!(colnames(logExps) %in% bootstrap)])
}
