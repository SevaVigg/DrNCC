...
logExpsWT	<- logExps[, setdiff( seq_along(colnames(logExps)), grep("(m618|mitfa)", colnames(logExps)))]
...
seedDir	<- file.path( resDir, paste0("s", seed))
dir.create( seedDir, showWarnings = FALSE)

set.seed( as.integer(Sys.time()))
logExpsImp <- DrImpute(logExpsWT)
