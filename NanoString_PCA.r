...
plotDir <- file.path(getwd(), "Plot", "PCA")
...
png( paste0( plotDir, .Platform$file.sep, "GeneProbeDendroNotNormalized.png"))
	par(cex = 0.7)
	plot( hclust( dist( log (Genes +1))), main = "Cluster Dendrograme: Genes and NanoString Probes; No normalization", xlab = "Complete linkage")
dev.off()

png( paste0( plotDir, .Platform$file.sep, "GeneDendroNormalized.png"))
	par(cex = 0.7)
	plot( hclust( dist( log (Genes_nh +1))), main = "Cluster Dendrograme: Genes normalized for houskeeping", xlab = "Complete linkage")
dev.off()
...
plotFileName<-file.path( getwd(), "Plot", paste(type, "_PC1_PC2.png" ,sep=""))
...
plotFileName<-file.path( getwd(), "Plot", paste(type, "_PC1_PC3.png" ,sep=""))
png(filename = plotFileName)
...
plotFileName<-file.path( getwd(), "Plot", paste(type, "_PC1_PC4.png" ,sep=""))
png(filename = plotFileName)
